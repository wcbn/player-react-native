...
declare module 'react-native-render-html' {
  ...
  namespace HTML {
    ...
    interface PassProps<P = Record<string, unknown>> {
      onLinkPress?: (
    ...
    type RendererFunction<P = Record<string, unknown>> = (
      htmlAttribs: HtmlAttributesDictionary,
      children: ReactNode,
      convertedCSSStyles: NonRegisteredStylesProp<any>,
      passProps: PassProps<P>
    ) => ReactNode
    type RendererDeclaration<P = Record<string, unknown>> =
      | RendererFunction<P>
      | { renderer: RendererFunction<P>; wrapper: 'Text' | 'View' }
    interface RendererDictionary<P = Record<string, unknown>> {
    ...
    interface ContainerProps<P = Record<string, unknown>> {
      /**
